...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for d in livingGhostPositionDistributions:
        	temp = self.distancer.getDistance(pacmanPosition, d.argMax())
        	ghostDistance.update({temp : d.argMax()})

        ghost_idx = ghostDistance.get(min(ghostDistance))

        a = {}
        for action in legal:
        	successorPosition = Actions.getSuccessor(pacmanPosition, action)
        	temp = self.distancer.getDistance(successorPosition, ghost_idx)
        	a.update({temp : action})
        bestAction = a.get(min(a))
